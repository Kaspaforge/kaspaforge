/* Kaspa Escrow support widget — Sara. Self-contained chat bubble that talks to /api/support/
   (nginx on the escrow domain routes it to the officeforge-support-escrow instance; canon = KASPA-ESCROW-FAQ.md).
   Fork of safe-widget.js: same logic, escrow-specific texts. EN/RU via <html lang>.
   No deps. Captcha (Cloudflare Turnstile) is used only if the backend reports captcha:on. */
(function () {
  ...
  if (window.__kescrowSara) return; window.__kescrowSara = true;
  var API = '/api/support';
  ...
  var T = RU ? {
    btn: 'Спросить Сару',
    sub: 'Поддержка Kaspa Гарант',
    hello: 'Привет! Я Сара, поддержка Kaspa Гарант 🙂 Спрашивай про сделки: создание, финансирование, релиз и возврат, споры, комиссии, восстановление доступа. Приватные ключи я никогда не спрашиваю — и ты их никому не показывай. Исход спора обещать не могу — это решает арбитр.',
    ph: 'Вопрос про сделку…',
    send: 'Отправить',
    ...
    btn: 'Ask Sara',
    sub: 'Kaspa Escrow support',
    hello: "Hi! I'm Sara, Kaspa Escrow support 🙂 Ask me anything about deals — creating one, funding, release and refund, disputes, fees, recovering access. I'll never ask for your private keys — don't show them to anyone. And I can't promise a dispute outcome — the arbiter rules on evidence.",
    ph: 'Ask about your deal…',
    send: 'Send',

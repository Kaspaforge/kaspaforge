...
/**
 * Profile master seed (32 bytes hex) — the root of deterministic derivation.
 * @returns {string}
 */
...
/**
 * Deterministic entity owner-token (hex 32 bytes) from the seed.
...
/**
 * Derived CHAT pair: like derive_keys, but with an even-parity pk (Kasia protocol).
 * @param {string} seed_hex
 * @param {string} domain
 * @param {number} index
 * @returns {string}
...
/**
 * Stray-payment sweep: a PLAIN UTXO accidentally sent straight to the vault's P2SH address is folded
 * into a fresh vault covenant UTXO via checkin (hot signature). utxos_json — ONE plain UTXO
...
/**
 * Migrate: both signatures (hot+alarm) = full owner authority — the whole UTXO goes INSTANTLY to to_addr
 * (moving to a new-version vault, rotating a compromised hot key, exit). Works from
 * both modes: unvault_dest_pk = Some(hex dest) for an UNVAULTING UTXO (mid-flight rescue).
 * Multiple vault UTXOs — JS loop, one at a time (single-input invariant), like sweep in the desk.
 * fee_sompi — explicit fee; if omitted → fund_fee(1) (=1M), as in build_split/build_sweep.
 * @param {string} cfg
 * @param {string} hot_sk
 ...
 * fee_sompi — explicit fee (from p2pk_fee); if omitted → half of fund_fee, as before.
...
/**
 * Deterministic keypair from the seed: domain ("vault/hot", "deal/key", …) + sequential index.
...
/**
 * arbitrate 100% to one party: to_buyer=true -> buyer, otherwise seller. Signed with the arbiter key.
 * @param {string} cfg
...
/**
 * Assemble the mutual tx from two signatures (buyer_sig, seller_sig hex) — ready for /api/safe/submit.
 * @param {string} cfg
 * @param {string} disputed_utxos
...
/**
 * ONE party's signature over the mutual tx (split) on the disputed UTXO. Both parties call with the same
 * `to_buyer` (from the verdict) → they sign an identical tx. `to_buyer` — a sompi string. -> { sig }.
 * @param {string} cfg
 * @param {string} disputed_utxos
 * @param {string} to_buyer

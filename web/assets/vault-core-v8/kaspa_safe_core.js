...
/**
 * Full set of safe keys — stays only in the user's browser.
 * @returns {string}
 */
...
/**
 * Seller reputation key (spec 2026-07-20): one public x-only identity per profile, derived
 * from the master seed — domain kaspaforge/v1/reputation/0, index pinned to 0 on purpose
 * (a bumpable index would be a cheap history reset). Even parity like every Kasia key.
 * @param {string} seed_hex
 * @returns {string}
 */
export function rep_identity(seed_hex) {
    let deferred3_0;
    let deferred3_1;
    try {
        const ptr0 = passStringToWasm0(seed_hex, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
        const len0 = WASM_VECTOR_LEN;
        const ret = wasm.rep_identity(ptr0, len0);
        var ptr2 = ret[0];
        var len2 = ret[1];
        if (ret[3]) {
            ptr2 = 0; len2 = 0;
...
/**
 * "I'm alive" check-in (hot). utxos_json — UTXO of the safe address.
 * @param {string} cfg
 * @param {string} hot_sk
...
/**
 * Deterministic Forge Sync credentials for copies of the same pre-Sync Desk profile.
 * The browser stores the returned Ed25519 seed as standards-compliant PKCS#8; the wallet key
 * itself is neither returned in this object nor sent to the mirror server.
 * @param {string} wallet_anchor_sk
 * @returns {string}
 */
export function forge_sync_identity(wallet_anchor_sk) {
    let deferred3_0;
    let deferred3_1;
    try {
        const ptr0 = passStringToWasm0(wallet_anchor_sk, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
        const len0 = WASM_VECTOR_LEN;
        const ret = wasm.forge_sync_identity(ptr0, len0);
        var ptr2 = ret[0];
        var len2 = ret[1];
        if (ret[3]) {
            ptr2 = 0; len2 = 0;
...
/**
 * Deterministic entity owner-token (hex, 32 bytes) from a seed.
 * @param {string} seed_hex
 * @param {string} domain
 * @param {number} index
 * @returns {string}
 */
export function derive_token(seed_hex, domain, index) {
    let deferred4_0;
    let deferred4_1;
    try {
        const ptr0 = passStringToWasm0(seed_hex, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
        ...
        const len1 = WASM_VECTOR_LEN;
        const ret = wasm.derive_token(ptr0, len0, ptr1, len1, index);
        var ptr3 = ret[0];
        var len3 = ret[1];
        if (ret[3]) {
            ptr3 = 0; len3 = 0;
...
/**
 * AUTO-inheritance (unsigned; safe with auto_inherit). utxos_json — UTXO of the safe address.
 * @param {string} cfg
 * @param {string} utxos_json
 * @param {string} network
 * @returns {string}
 ...
 * @returns {string}
 */
export function encrypt_profile(json, passphrase) {
    let deferred4_0;
    let deferred4_1;
    try {
        ...
        const len1 = WASM_VECTOR_LEN;
        const ret = wasm.encrypt_profile(ptr0, len0, ptr1, len1);
        var ptr3 = ret[0];
        var len3 = ret[1];
        if (ret[3]) {
            ptr3 = 0; len3 = 0;
...
/**
 * Derived CHAT pair: like derive_keys, but pk has even parity (Kasia protocol).
 * @param {string} seed_hex
 * @param {string} domain
 * @param {number} index
 * @returns {string}
 */
export function derive_chat_keys(seed_hex, domain, index) {
    let deferred4_0;
    let deferred4_1;
    try {
        const ptr0 = passStringToWasm0(seed_hex, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
        ...
        const ptr1 = passStringToWasm0(domain, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
        const len1 = WASM_VECTOR_LEN;
        const ret = wasm.derive_chat_keys(ptr0, len0, ptr1, len1, index);
        var ptr3 = ret[0];
        var len3 = ret[1];
        if (ret[3]) {
            ptr3 = 0; len3 = 0;
...
/**
 * Sweep a stray payment: a PLAIN UTXO that accidentally landed directly on the safe's P2SH address
 * is folded into a fresh safe covenant UTXO via checkin (hot signature). utxos_json — ONE plain UTXO
 * (covenant_id absent). One at a time (single-input invariant).
 * @param {string} cfg
 * @param {string} hot_sk
 * @param {string} utxos_json
 * @param {string} network
 * @returns {string}
 */
export function build_vault_sweep(cfg, hot_sk, utxos_json, network) {
    let deferred6_0;
    let deferred6_1;
    try {
        const ptr0 = passStringToWasm0(cfg, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
        ...
        const ptr3 = passStringToWasm0(network, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
        const len3 = WASM_VECTOR_LEN;
        const ret = wasm.build_vault_sweep(ptr0, len0, ptr1, len1, ptr2, len2, ptr3, len3);
        var ptr5 = ret[0];
        var len5 = ret[1];
        if (ret[3]) {
            ptr5 = 0; len5 = 0;
...
/**
 * MANUAL inheritance (heir signature; safe without auto_inherit).
 * @param {string} cfg
 ...
 * fee_sompi — explicit fee (from p2pk_fee); if not passed → floor fund_fee, as before.
 * @param {string} funding_sk
 * @param {string} to_addr
 * @param {string} utxos_json
 * @param {string} _network
 * @param {string | null} [fee_sompi]
 * @returns {string}
 */
export function build_sweep(funding_sk, to_addr, utxos_json, _network, fee_sompi) {
    let deferred7_0;
    let deferred7_1;
    try {
        ...
        const len3 = WASM_VECTOR_LEN;
        var ptr4 = isLikeNone(fee_sompi) ? 0 : passStringToWasm0(fee_sompi, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
        var len4 = WASM_VECTOR_LEN;
        const ret = wasm.build_sweep(ptr0, len0, ptr1, len1, ptr2, len2, ptr3, len3, ptr4, len4);
        var ptr6 = ret[0];
        var len6 = ret[1];
        if (ret[3]) {
            ptr6 = 0; len6 = 0;
...
/**
 * P2PK transfer fee for this UTXO set at the given feerate (sompi/gram, from the
 * node's /api/safe/fee-estimate). Single source of truth: "Max" in the desk subtracts exactly
 * what the builder will take. Returns sompi as a string (u64 doesn't fit a JS number losslessly).
 * @param {string} utxos_json
 * @param {number} feerate
 * @returns {string}
 */
export function p2pk_fee(utxos_json, feerate) {
    let deferred3_0;
    let deferred3_1;
    try {
        const ptr0 = passStringToWasm0(utxos_json, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
        const len0 = WASM_VECTOR_LEN;
        const ret = wasm.p2pk_fee(ptr0, len0, feerate);
        var ptr2 = ret[0];
        var len2 = ret[1];
        if (ret[3]) {
            ptr2 = 0; len2 = 0;
 ...
 * @returns {string}
 */
export function decrypt_profile(armored, passphrase) {
    let deferred4_0;
    let deferred4_1;
    try {
        ...
        const ptr1 = passStringToWasm0(passphrase, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
        const len1 = WASM_VECTOR_LEN;
        const ret = wasm.decrypt_profile(ptr0, len0, ptr1, len1);
        var ptr3 = ret[0];
        var len3 = ret[1];
        if (ret[3]) {
            ptr3 = 0; len3 = 0;
...
/**
 * Complete withdrawal after the timelock (unsigned, strictly to dest).
 * @param {string} cfg
 * @param {string} dest_pk
 * @param {string} utxos_json
 * @param {string} network
...
/**
 * Funding: all UTXOs of the funding address -> safe (genesis covenant).
 * @param {string} cfg
 * @param {string} funding_sk
 * @param {string} utxos_json
 * @param {string} network
...
/**
 * BIP340 signature by the reputation key over sha256(msg) — `msg_hex` is the full message
 * (ASCII domain tag + payload) assembled by the caller; the server verifies the identical
 * digest (rep.rs::verify_sig). The private key never leaves the WASM boundary.
 * @param {string} seed_hex
 * @param {string} msg_hex
 * @returns {string}
 */
export function rep_sign(seed_hex, msg_hex) {
    let deferred4_0;
    let deferred4_1;
    try {
        const ptr0 = passStringToWasm0(seed_hex, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
        ...
        const len1 = WASM_VECTOR_LEN;
        const ret = wasm.rep_sign(ptr0, len0, ptr1, len1);
        var ptr3 = ret[0];
        var len3 = ret[1];
        if (ret[3]) {
            ptr3 = 0; len3 = 0;
 ...
 * @returns {boolean}
 */
export function is_encrypted(s) {
...
/**
 * Cancel (alarm). utxos_json — UTXO of the UNVAULTING address.
 * @param {string} cfg
...
/**
 * Deterministic keypair from a seed: domain ("vault/hot", "deal/key", …) + sequential index.
 * JSON {sk, pk}. Same arguments → same key always (see derive.rs).
 * @param {string} seed_hex
 * @param {string} domain
 * @param {number} index
 * @returns {string}
 */
export function derive_keys(seed_hex, domain, index) {
    let deferred4_0;
    let deferred4_1;
    try {
        const ptr0 = passStringToWasm0(seed_hex, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
        ...
        const ptr1 = passStringToWasm0(domain, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
        const len1 = WASM_VECTOR_LEN;
        const ret = wasm.derive_keys(ptr0, len0, ptr1, len1, index);
        var ptr3 = ret[0];
        var len3 = ret[1];
        if (ret[3]) {
            ptr3 = 0; len3 = 0;
...
/**
 * Spend UTXOs from every derived address of one HD wallet in a single transaction. Private keys
 * and UTXOs stay inside the browser/WASM boundary; only the signed transaction is submitted.
 * @param {string} sources_json
 * @param {string} to_addr
 * @param {string} change_addr
 * @param {string} amount
 * @param {string} fee_sompi
 * @returns {string}
 */
export function build_wallet_split(sources_json, to_addr, change_addr, amount, fee_sompi) {
    let deferred7_0;
    let deferred7_1;
    try {
        ...
        const ptr1 = passStringToWasm0(to_addr, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
        const len1 = WASM_VECTOR_LEN;
        const ptr2 = passStringToWasm0(change_addr, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
        const len2 = WASM_VECTOR_LEN;
        const ptr3 = passStringToWasm0(amount, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
        const len3 = WASM_VECTOR_LEN;
        const ptr4 = passStringToWasm0(fee_sompi, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
        const len4 = WASM_VECTOR_LEN;
        const ret = wasm.build_wallet_split(ptr0, len0, ptr1, len1, ptr2, len2, ptr3, len3, ptr4, len4);
        var ptr6 = ret[0];
        var len6 = ret[1];
        if (ret[3]) {
            ptr6 = 0; len6 = 0;
...
/**
 * Initiate withdrawal (hot). utxos_json — UTXO of the safe address.
 * @param {string} cfg
 * @param {string} hot_sk
 * @param {string} dest_pk
 * @param {string} utxos_json
...
/**
 * Split the funding address balance: amount to to_addr, change back (desk wallet, faucet).
 * fee_sompi — explicit fee (from p2pk_fee); if not passed → floor fund_fee, as before.
 * @param {string} funding_sk
 * @param {string} to_addr
 * @param {string} amount
 ...
 * @returns {string}
 */
export function build_split(funding_sk, to_addr, amount, utxos_json, _network, fee_sompi) {
    let deferred8_0;
    let deferred8_1;
    try {
        const ptr0 = passStringToWasm0(funding_sk, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
        ...
        const len4 = WASM_VECTOR_LEN;
        var ptr5 = isLikeNone(fee_sompi) ? 0 : passStringToWasm0(fee_sompi, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
        var len5 = WASM_VECTOR_LEN;
        const ret = wasm.build_split(ptr0, len0, ptr1, len1, ptr2, len2, ptr3, len3, ptr4, len4, ptr5, len5);
        var ptr7 = ret[0];
        var len7 = ret[1];
        if (ret[3]) {
            ptr7 = 0; len7 = 0;
...
/**
 * Migrate: both signatures (hot+alarm) = full owner authority — the entire UTXO INSTANTLY to to_addr
 * (moving to a new-version safe, rotating a compromised hot key, exiting). Works from
 * both modes: unvault_dest_pk = Some(hex dest) for an UNVAULTING UTXO (mid-flight rescue).
 * Multiple UTXOs on the safe — JS loop one at a time (single-input invariant), same as sweep in the desk.
 * fee_sompi — explicit fee; if not passed → fund_fee(1) (=1M), same as in build_split/build_sweep.
 * @param {string} cfg
 * @param {string} hot_sk
 * @param {string} alarm_sk
 * @param {string} utxos_json
 * @param {string} to_addr
 * @param {string} _network
 * @param {string | null} [unvault_dest_pk]
 * @param {string | null} [fee_sompi]
 * @returns {string}
 */
export function build_migrate(cfg, hot_sk, alarm_sk, utxos_json, to_addr, _network, unvault_dest_pk, fee_sompi) {
    let deferred10_0;
    let deferred10_1;
    try {
        const ptr0 = passStringToWasm0(cfg, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
        ...
        const ptr1 = passStringToWasm0(hot_sk, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
        const len1 = WASM_VECTOR_LEN;
        const ptr2 = passStringToWasm0(alarm_sk, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
        const len2 = WASM_VECTOR_LEN;
        const ptr3 = passStringToWasm0(utxos_json, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
        const len3 = WASM_VECTOR_LEN;
        const ptr4 = passStringToWasm0(to_addr, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
        const len4 = WASM_VECTOR_LEN;
        const ptr5 = passStringToWasm0(_network, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
        const len5 = WASM_VECTOR_LEN;
        var ptr6 = isLikeNone(unvault_dest_pk) ? 0 : passStringToWasm0(unvault_dest_pk, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
        var len6 = WASM_VECTOR_LEN;
        var ptr7 = isLikeNone(fee_sompi) ? 0 : passStringToWasm0(fee_sompi, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
        var len7 = WASM_VECTOR_LEN;
        const ret = wasm.build_migrate(ptr0, len0, ptr1, len1, ptr2, len2, ptr3, len3, ptr4, len4, ptr5, len5, ptr6, len6, ptr7, len7);
        var ptr9 = ret[0];
        var len9 = ret[1];
        if (ret[3]) {
            ptr9 = 0; len9 = 0;
...
/**
 * Profile master seed (32 bytes hex) — root of deterministic derivation.
 * @returns {string}
 */
...
/**
 * release (buyer confirms): everything to the seller + fee. utxos_json — UTXO of the escrow address
 * (ACTIVE or DISPUTED — a party can release even during a dispute as a settlement).
 * @param {string} cfg
...
/**
 * autoRelease (unsigned, sent by the server/anyone): dispute window elapsed -> to seller.
 * @param {string} cfg
 * @param {string} utxos_json
 * @param {string} network
 * @returns {string}
...
/**
 * arbitrateSplit: `to_buyer` sompi to the buyer, the rest to the seller, + fee. Both shares >= MIN_OUT (1 KAS).
 * @param {string} cfg
 * @param {string} disputed_utxos
 * @param {string} to_buyer
 * @param {string} arbiter_sk
 * @returns {string}
 */
export function escrow_arbitrate_split(cfg, disputed_utxos, to_buyer, arbiter_sk) {
    let deferred6_0;
    let deferred6_1;
    try {
        const ptr0 = passStringToWasm0(cfg, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
        const len0 = WASM_VECTOR_LEN;
        const ptr1 = passStringToWasm0(disputed_utxos, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
        const len1 = WASM_VECTOR_LEN;
        const ptr2 = passStringToWasm0(to_buyer, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
        const len2 = WASM_VECTOR_LEN;
        const ptr3 = passStringToWasm0(arbiter_sk, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
        const len3 = WASM_VECTOR_LEN;
        const ret = wasm.escrow_arbitrate_split(ptr0, len0, ptr1, len1, ptr2, len2, ptr3, len3);
        var ptr5 = ret[0];
        var len5 = ret[1];
        if (ret[3]) {
            ptr5 = 0; len5 = 0;
...
/**
 * One party's deal key — stays only in their browser.
 * @returns {string}
 */
...
/**
 * dispute (buyer opens a dispute): ACTIVE -> DISPUTED, amount is preserved.
 * @param {string} cfg
 * @param {string} buyer_sk
 * @param {string} utxos_json
 * @param {string} network
...
/**
 * Build the mutual-tx from two signatures (buyer_sig, seller_sig hex) — ready for /api/safe/submit.
 * @param {string} cfg
 * @param {string} disputed_utxos
 * @param {string} to_buyer
 * @param {string} buyer_sig
 * @param {string} seller_sig
 * @returns {string}
 */
export function escrow_mutual_combine(cfg, disputed_utxos, to_buyer, buyer_sig, seller_sig) {
    let deferred7_0;
    let deferred7_1;
    try {
        const ptr0 = passStringToWasm0(cfg, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
        ...
        const ptr2 = passStringToWasm0(to_buyer, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
        const len2 = WASM_VECTOR_LEN;
        const ptr3 = passStringToWasm0(buyer_sig, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
        const len3 = WASM_VECTOR_LEN;
        const ptr4 = passStringToWasm0(seller_sig, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
        const len4 = WASM_VECTOR_LEN;
        const ret = wasm.escrow_mutual_combine(ptr0, len0, ptr1, len1, ptr2, len2, ptr3, len3, ptr4, len4);
        var ptr6 = ret[0];
        var len6 = ret[1];
        if (ret[3]) {
            ptr6 = 0; len6 = 0;
...
/**
 * One party's signature over the mutual-tx (split) on the disputed UTXO. Both sides call with the
 * same `to_buyer` (from the verdict) → sign an identical tx. `to_buyer` is a sompi string. -> { sig }.
 * @param {string} cfg
 * @param {string} disputed_utxos
 * @param {string} to_buyer
 * @param {string} signer_sk
 * @returns {string}
 */
export function escrow_mutual_sig(cfg, disputed_utxos, to_buyer, signer_sk) {
    let deferred6_0;
    let deferred6_1;
    try {
        const ptr0 = passStringToWasm0(cfg, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
        ...
        const ptr2 = passStringToWasm0(to_buyer, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
        const len2 = WASM_VECTOR_LEN;
        const ptr3 = passStringToWasm0(signer_sk, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
        const len3 = WASM_VECTOR_LEN;
        const ret = wasm.escrow_mutual_sig(ptr0, len0, ptr1, len1, ptr2, len2, ptr3, len3);
        var ptr5 = ret[0];
        var len5 = ret[1];
        if (ret[3]) {
            ptr5 = 0; len5 = 0;
...
/**
 * Funding: UTXO of the buyer's funding address -> escrow covenant (genesis).
 * @param {string} cfg
 * @param {string} funding_sk
 * @param {string} utxos_json
 * @param {string} network
...
/**
 * P2SH address of the escrow deal in ACTIVE (both sides compute and cross-check it).
 * @param {string} cfg
 * @param {string} network
 * @returns {string}
 */
...
/**
 * timeout (unsigned): arbiter didn't render a verdict in time -> the timeout_to party.
 * @param {string} cfg
 * @param {string} utxos_json
 * @param {string} network
 * @returns {string}
...
/**
 * P2SH address of the escrow deal in DISPUTED (the panel watches it after a dispute opens).
 * @param {string} cfg
 * @param {string} network
 * @returns {string}
 */
...
/**
 * arbitrate 100% to one side: to_buyer=true -> buyer, otherwise seller. Signed with the arbiter's key.
 * @param {string} cfg
 * @param {string} disputed_utxos
 * @param {boolean} to_buyer
 * @param {string} arbiter_sk
 * @returns {string}
 */
export function escrow_arbitrate_to(cfg, disputed_utxos, to_buyer, arbiter_sk) {
    let deferred5_0;
    let deferred5_1;
    try {
        const ptr0 = passStringToWasm0(cfg, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
        ...
        const ptr1 = passStringToWasm0(disputed_utxos, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
        const len1 = WASM_VECTOR_LEN;
        const ptr2 = passStringToWasm0(arbiter_sk, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
        const len2 = WASM_VECTOR_LEN;
        const ret = wasm.escrow_arbitrate_to(ptr0, len0, ptr1, len1, to_buyer, ptr2, len2);
        var ptr4 = ret[0];
        var len4 = ret[1];
        if (ret[3]) {
            ptr4 = 0; len4 = 0;
...
/**
 * Chat key address (kaspa:…) — fund it to send messages.
 * @param {string} pk_hex
 * @param {string} network
 * @returns {string}
 */
...
/**
 * Build a message: signed comm-tx (payload on-chain) + ciphertexts for the server relay.
 * Returns JSON: { tx (for /api/safe/submit), comm_hex (ciphertext to the receiver), self_hex (self copy) }.
 ...
 * only inside an encrypted chat message (chat_build_send) to the receiver.
 * @returns {string}
 */
...
/**
 * Encrypt file bytes with a key (hex) -> cipher blob (nonce||ct) for upload to the server.
...
/**
 * Decrypt a media cipher blob with a key (hex) -> file bytes.
 * @param {string} key_hex

/* Kaspa Safe legacy service worker — stage 6 cutover: self-destroying, and no page registers it.
...
self.addEventListener('activate', (event) => {
  event.waitUntil((async () => {
    // Only this worker's own caches (ksafe-*): caches is origin-wide, so a blind wipe here would
    // also delete the React desk's workbox precache.
    const keys = await caches.keys();
    await Promise.all(keys.filter((key) => key.startsWith('ksafe-')).map((key) => caches.delete(key)));
